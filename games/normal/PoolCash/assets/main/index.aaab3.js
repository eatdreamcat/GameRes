        ...
        _proto.ballDrop = function ballDrop(model, pocket) {
          ...
          if (model.isWhiteBall) {
            if (GameStateController.inst.isGameOver()) {
              this.balls.remove(this.whiteBall.ID);
              this.whiteBall.removeSelf();
              return;
            }
    ...
    execute: function () {
      ...
      var VersionLabel = exports('VersionLabel', (_dec = ccclass("VersionLabel"), _dec(_class = /*#__PURE__*/function (_Component) {
        ...
        _proto.start = function start() {
          ...
          this.getComponent(UIOpacity).opacity = 166;
...
System.register("chunks:///_virtual/BallObject.ts", ['./_rollupPluginModLoBabelHelpers.js', 'cc', './GameRule.ts', './Signal2.ts', './Signal3.ts', './Cocos.ts', './TableManager.ts', './MaterialFactory.ts', './table.ts', './BorderCollider.ts', './Pole.ts', './PrefabView.ts', './GameLogic.ts'], function (exports) {
  ...
  var _inheritsLoose, _createClass, cclegacy, _decorator, v3, MeshRenderer, PhysicsSystem, Tween, math, tween, Vec3, SphereCollider, BoxCollider, director, Director, RigidBody, EasyMode, BaseSignal, ShotSignal, Clamp, TableManager, MaterialFactory, Physical_ID, BorderCollider, BorderType, HitBorderSignal, Pole, UpdateShotLineSignal, PrefabView, GameLogic, WhiteBallReposSignal, NextRountSignal, BallReturnSignal;
  ...
  return {
    ...
    execute: function () {
      ...
      var MoveSpeed = 15;
      var BallObject = exports('BallObject', (_dec = ccclass("BallObject"), _dec(_class = (_temp = _class2 = /*#__PURE__*/function (_PrefabView) {
        ...
        _proto.onReuse = function onReuse() {
          if (this.model) {
            this.initRenderer();
            this.initPhysic();
            this.node.getChildByName("Cube").active = false;
          }
        };
        ...
        _proto.onReposition = function onReposition(position, callback) {
          ...
          if (this.model && this.model.isWhiteBall) {
            ...
            Tween.stopAllByTarget(this.node);
            var rotation = math.quat();
            var time = Vec3.distance(this.node.position, position) / MoveSpeed;
            math.Quat.fromEuler(rotation, 0, 180, 0);
            var ballWorld = v3();
            ...
            if (math.approx(time, 0, 0.01)) {
              ...
              tween(this.node).to(0, {
                ...
                _this3.node.getWorldPosition(ballWorld);
        ...
        _proto.onCollide = function onCollide(a) {
          ...
          if (a.selfCollider instanceof SphereCollider && a.otherCollider instanceof SphereCollider) {
            this.RigidBody.angularFactor = v3(1, 1, 1);
            this.RigidBody.linearFactor = v3(1, 1, 1);
            this.angularFactor = v3(0, 0, 0); // console.log(
            //   "on collide:",
            //   this.model.Point,
            //   ",this.isSleep:",
            //   this.isSleep,
            ...
            BallHitSignal.inst.dispatch(this.model, otherBallObj.model, Clamp(velocity.length() * this.RigidBody.mass, 10, 0));
            ...
            if (this.model.isWhiteBall && Pole.HitNormal && this.shotVelocity && this.shotImpulse && EasyMode()) {
              ...
              var velocityOther = v3();
              otherBallObj.RigidBody.getLinearVelocity(velocityOther);
              var impulse = Pole.HitNormal.clone().normalize().multiplyScalar(this.shotImpulse.length()).multiplyScalar(ballOtherPhysic.Damping * ballPhysicWhite.Damping * (math.approx(velocityOther.length(), 0, 0.0001) ? 0 : Math.abs(velocityOther.length() / this.shotVelocity.length())));
              impulse.y = 0;
              CC_DEBUG && console.log("shot impulse:", this.shotImpulse, " orther impulse:", impulse, ",velocity:", velocity, ",shotVelocity:", this.shotVelocity, ", ratio:", Math.abs(velocityOther.length() / this.shotVelocity.length()), ", hitNormal:", Pole.HitNormal, ", otherVelocity:", velocityOther);
              otherBallObj.RigidBody.angularFactor = v3(1, 0, 1);
          ...
          if (a.selfCollider instanceof SphereCollider && a.otherCollider instanceof BoxCollider) {
            ...
            if (borderCollider) {
              ...
              HitBorderSignal.inst.dispatch(borderCollider.type, Clamp(velocity.length() / 10 * this.RigidBody.mass, 8, 0));
            }
          }
        };
        ...
        _proto.onShot = function onShot(power, angleY, forward) {
          ...
          this.isSleep = true;
          this.power = power;
          if (this.model.isWhiteBall == false) return; // let rotation = v3();
          // this.node.rotation.getEulerAngles(rotation);
          // this.node.setRotationFromEuler(rotation.x, 180 + angleY, rotation.z);
          ...
          this.angularFactor = v3(Math.abs(forward.z), 0, Math.abs(forward.x));
          this.RigidBody.applyLocalImpulse(v3(0, 0, powerData.BaseImpulse + power * powerData.ExtraImpulse));
          /**
           * 施加扭转力
          ...
          console.log("shot power:", power, "forward:", forward, forward.clone().normalize(), "angleY:", angleY, "impulse:", impulse);
        };
        ...
        _proto.onRotation = function onRotation(rotation) {
          if (!this.model || !this.model.isWhiteBall) return;
          var rotationSelf = v3();
          this.node.rotation.getEulerAngles(rotationSelf);
          this.node.setRotationFromEuler(rotationSelf.x, 180 + rotation.y, rotationSelf.z); // console.log("ball forward:", this.node.forward);
        };

        _proto.initEvent = function initEvent() {
          ShotSignal.inst.removeTarget(this);
          ShotSignal.inst.addListener(this.onShot, this);
          UpdateShotLineSignal.inst.removeTarget(this);
          UpdateShotLineSignal.inst.addListener(this.onRotation, this);
          WhiteBallReposSignal.inst.removeTarget(this);
          WhiteBallReposSignal.inst.addListener(this.onReposition, this);
          NextRountSignal.inst.removeTarget(this);
          NextRountSignal.inst.addListener(this.onNextRound, this);
          BallReturnSignal.inst.removeTarget(this);
          BallReturnSignal.inst.addListener(this.onReturn, this);
          director.targetOff(this);
          director.on(Director.EVENT_PHYSIC_START_EMIT_EVENT, this.onPhysicEmitEvent, this);
          this.Collider.targetOff(this);
        ...
        _proto.onReturn = function onReturn(callback) {
          ...
          Tween.stopAllByTarget(this.node);
          var rotation = math.quat();
          var time = Vec3.distance(this.node.position, this.model.Position) / MoveSpeed;
          math.Quat.fromEuler(rotation, 0, 180, 0);
          BallObject.delayTime = Math.max(time + 0.1, BallObject.delayTime);
          this.unscheduleAllCallbacks();
          this.scheduleOnce(function () {
            if (callback && _this4.model.isWhiteBall) callback();
          }, BallObject.delayTime);
          ...
          if (math.approx(time, 0, 0.01)) {
            ...
            //   })
            //   .start();
            // console.error("model:", this.model.Point, this.model.Position);
            tween(this.node).to(time, {
              position: this.model.Position
            }).call(function () {
              _this4.RigidBody.enabled = true;
              _this4.Collider.enabled = true;

              _this4.RigidBody.clearState();
            }).start();
          }
        };

        _proto.onPhysicEmitEvent = function onPhysicEmitEvent() {// if (
          //   this.model &&
          //   this.model.isWhiteBall &&
          //   !Vec3.equals(this.angularFactor, Vec3.ZERO)
          // ) {
          //   let velocity = v3();
          //   this.RigidBody.getAngularVelocity(velocity);
          //   if (this.angularFactor.x == 0) {
          //     this.RigidBody.setAngularVelocity(v3(0, 0, velocity.z));
          //   } else {
          //     this.RigidBody.setAngularVelocity(
          //       v3(
          //         velocity.x,
          //         0,
          //         (-velocity.x * this.angularFactor.z) / this.angularFactor.x
          //       )
          //     );
          //   }
          //   let velocity2 = v3();
          //   this.RigidBody.getAngularVelocity(velocity2);
          //   console.log(
          //     "velocity:",
          //     velocity,
          //     "velocity2:",
        ...
        return BallObject;
      }(PrefabView), _class2.delayTime = 0, _temp)) || _class));
...
System.register("chunks:///_virtual/BorderCollider.ts", ['./_rollupPluginModLoBabelHelpers.js', 'cc', './Signal2.ts', './Signal3.ts', './TableManager.ts', './CelerSDK.ts', './table.ts', './TutorialRotation.ts', './GameLogic.ts'], function (exports) {
  'use strict';

  var _inheritsLoose, _applyDecoratedDescriptor, _initializerDefineProperty, _assertThisInitialized, _createClass, cclegacy, _decorator, Enum, MeshRenderer, PhysicsSystem, Collider, Component, BaseSignal, GameReadySignal, TableManager, CelerSDK, Physical_ID, TutorialStep, TutorialStepSignal;
      ...
      var BorderCollider = exports('BorderCollider', (_dec = ccclass("BorderCollider"), _dec2 = property({
        ...
        _proto.onReady = function onReady() {
          if (this.type == BorderType.HoleChecker || this.type == BorderType.TutorialChecker) {
            ...
            if (this.type == BorderType.TutorialChecker && CelerSDK.inst.isNew() == false) {
          ...
          switch (this.type) {
            ...
            case BorderType.HoleBorder:
              var physicFloo2r = TableManager.inst.getPhysical(Physical_ID.FangZhiTiaoQiuDangBan);
    ...
    execute: function () {
      ...
      var WhiteShotLine = exports('WhiteShotLine', (_dec = ccclass("WhiteShotLine"), _dec2 = property(Node), _dec3 = property(Node), _dec(_class = (_class2 = (_temp = /*#__PURE__*/function (_Component) {
        ...
        _proto.start = function start() {
          ...
          this.LeftStart.active = CC_PREVIEW;
          this.RightStart.active = CC_PREVIEW;
          this.CenterStart.active = CC_PREVIEW;
          this.HitPoint.active = CC_PREVIEW;
          this.TestRayStart.active = CC_PREVIEW; // Your initialization goes here.
  ...
  return {
    ...
    execute: function () {
      ...
      function GetTotalHeartCount() {
        return CC_PREVIEW ? 1000 : 5;
      }

      function StreakBonusLimit() {
        return CC_PREVIEW ? 3 : 3;
      }
    ...
    execute: function () {
      ...
      var ResultLayerView = exports('ResultLayerView', (_dec = ccclass("ResultLayerView"), _dec2 = property(SpriteAtlas), _dec(_class = (_class2 = (_temp = /*#__PURE__*/function (_BaseView) {
        ...
        _proto.showInfo = function showInfo() {
          ...
          this.Score.onNumberChanged(PlayModel.inst.getScoreByType(ScoreType.NoBust) + PlayModel.inst.getScoreByType(ScoreType.PauseCost) + PlayModel.inst.getScoreByType(ScoreType.Pocket), function () {
            _this3.Count++;
            console.log("Score Done");
          });
          this.HeartBonus.onNumberChanged(PlayModel.inst.getScoreByType(ScoreType.HeartBonus), function () {
            _this3.Count++;
            console.log("HeartBonus Done");
          });
          /** 技巧得分 */
        ...
        _createClass(ResultLayerView, [{
          ...
          key: "HeartBonus",
          get: function get() {
            return this.LabelRoot.getChildByName("HeartBonus").getComponent(NumberChangedView);
          }
        }, {
          key: "TrickScore",
          get: function get() {
...
System.register("chunks:///_virtual/App.ts", ['./_rollupPluginModLoBabelHelpers.js', 'cc', './PlayModel.ts', './InitialFacade.ts', './ResourceController.ts'], function (exports) {
  ...
  var _applyDecoratedDescriptor, _inheritsLoose, _initializerDefineProperty, _assertThisInitialized, cclegacy, _decorator, Material, SpriteAtlas, Node, PhysicsSystem, v3, setDisplayStats, game, Component, PlayModel, InitialFacade, ResourceController, AnimationType;
...
System.register("chunks:///_virtual/AdaptationWigetView.ts", ['./_rollupPluginModLoBabelHelpers.js', 'cc'], function (exports) {
  ...
  var _applyDecoratedDescriptor, _inheritsLoose, _initializerDefineProperty, _assertThisInitialized, _createClass, cclegacy, _decorator, Enum, v3, sys, view, size, UITransform, Component;
    ...
    execute: function () {
      ...
      var AdaptationWigetView = exports('AdaptationWigetView', (_dec = ccclass("AdaptationWigetView"), _dec2 = property({
        ...
        _proto.alignTop = function alignTop() {
          /** 实际尺寸 */
          var canvasSize = size((sys.getSafeAreaRect().width + sys.getSafeAreaRect().x) * this.UseRatio, (sys.getSafeAreaRect().height + sys.getSafeAreaRect().y) * this.UseRatio); // console.error(
          //   sys.getSafeAreaRect(),
          //   this.UseRatio,
          //   view.getCanvasSize(),
          //   canvasSize
        ...
        _proto.alignTopLeft = function alignTopLeft() {
          var canvasSize = size((sys.getSafeAreaRect().width + sys.getSafeAreaRect().x) * this.UseRatio, (sys.getSafeAreaRect().height + sys.getSafeAreaRect().y) * this.UseRatio); // this.node.getComponent(UITransform).anchorX = 0;
          // this.node.getComponent(UITransform).anchorY = 1;
        ...
        _proto.alignTopRight = function alignTopRight() {
          var canvasSize = size((sys.getSafeAreaRect().width + sys.getSafeAreaRect().x) * this.UseRatio, (sys.getSafeAreaRect().height + sys.getSafeAreaRect().y) * this.UseRatio); // this.node.getComponent(UITransform).anchorX = 1;
          // this.node.getComponent(UITransform).anchorY = 1;
        ...
        _proto.alignBottom = function alignBottom() {
          /** 实际尺寸 */
          var canvasSize = size((sys.getSafeAreaRect().width + sys.getSafeAreaRect().x) * this.UseRatio, (sys.getSafeAreaRect().height + sys.getSafeAreaRect().y) * this.UseRatio); // this.node.getComponent(UITransform).anchorX = 0.5;
        ...
        _proto.alignLeft = function alignLeft() {
          ...
          var canvasSize = size((sys.getSafeAreaRect().width + sys.getSafeAreaRect().x) * this.UseRatio, (sys.getSafeAreaRect().height + sys.getSafeAreaRect().y) * this.UseRatio);
...
System.register("chunks:///_virtual/HelpTextView.ts", ['./_rollupPluginModLoBabelHelpers.js', 'cc', './Signal3.ts', './table.ts', './TutorialRotation.ts', './GameLogic.ts'], function (exports) {
  'use strict';

  var _inheritsLoose, _createClass, cclegacy, _decorator, RichText, Label, Component, ShowHelpLayerSignal, En_View, En_ID, TutorialStep, TutorialStepSignal;
      ...
      var Pole = exports('Pole', (_dec = ccclass("Pole"), _dec(_class = (_temp = _class2 = /*#__PURE__*/function (_Component) {
        ...
        _proto.start = function start() {
          ...
          InitPolePositionSignal.inst.addListener(function (position) {
            console.log("InitPolePositionSignal");
            _this2.node.active = true;
...
System.register("chunks:///_virtual/AudioManager.ts", ['./_rollupPluginModLoBabelHelpers.js', 'cc', './ToSingleTon.ts', './Signal3.ts', './GameStateController.ts', './HashMap.ts', './BorderCollider.ts', './MainUI.ts', './Pole.ts', './BallObject.ts', './GameLogic.ts'], function (exports) {
  ...
  return {
    ...
    execute: function () {
      ...
      AudioController.canPlay = sys.Platform.WIN32 == sys.platform;
