  ...
  follow: [ function(require, module, exports) {
    ...
    cc.Class({
      ...
      addBird: function addBird(prefab) {
        ...
        this.newBird = newBird;
        this.addNewBird = true;
      },
      ...
      blockStoped: function blockStoped(block) {
        ...
        if (this.gameOn) {
          ...
          if (Math.abs(this.lastX - block.node.x) < true ? 20 : 2) {
            this.addScore(2);
      ...
      update: function update() {
        ...
        var HeroControl = this.target.getComponent("HeroControl");
        if (this.addNewBlock && this.newBlock && Math.abs(this.newBlock.x - HeroControl.node.x) <= 120 && true) {
          HeroControl.onTouchStart();
          this.addNewBlock = false;
          HeroControl.direction = 0;
        }
        if (this.addNewBird && this.newBird && Math.abs(this.newBird.x - HeroControl.node.x) <= 95 && true) {
          HeroControl.onTouchStart();
          this.addNewBird = false;
          HeroControl.direction = 0;
        }
      },
